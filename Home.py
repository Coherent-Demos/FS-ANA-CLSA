...
with col1:
    with st.expander("**Equity Analytics Models**", expanded=True):
        ...
        st.text("‎")
        st.markdown('<a href="/Equity_Analytics" class="custom-link" target="_self">Equity Analytics Model →</a>', unsafe_allow_html=True)
...
with col2:
    with st.expander("**Region Sector & Analytics Models**", expanded=True):
        ...
        st.text("‎")
        st.markdown('<a href="/Region_&_Sector_Analytics" class="custom-link" target="_self">Region & Sector Analytics Model →</a>', unsafe_allow_html=True)
